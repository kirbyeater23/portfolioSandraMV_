<template>
  ...
  <main class="seccionAbout">
    <section class="heroAbout" id="heroAbout">
    ...
    <section class="bioAbout" id="bioAbout">
      ...
      <div class="bioColumna">
        <small class="bioEtiqueta">{{ bio.es.etiqueta }}</small>
        <h3 class="bioSaludo">{{ bio.es.saludo }}</h3>
        <p v-for="(parrafo, i) in bio.es.parrafos" :key="'es' + i">{{ parrafo }}</p>
      </div>

      <div class="bioColumna">
        <small class="bioEtiqueta">{{ bio.en.etiqueta }}</small>
        <h3 class="bioSaludo">{{ bio.en.saludo }}</h3>
        <p v-for="(parrafo, i) in bio.en.parrafos" :key="'en' + i">{{ parrafo }}</p>
      </div>
    </section>
  </main>
...
onMounted(async () => {
  document.title = 'About | Sandra Martínez Villacorta'
  await nextTick()
  aplicarAnimaciones()
  aplicarScramble()
})
